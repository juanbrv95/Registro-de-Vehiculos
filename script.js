...
function mostrarTabla(filtro = "") {
    ...
    // 2. Ordenamos alfabéticamente por el campo "nombre" (A-Z)
    vehiculosFiltrados.sort((a, b) => a.nombre.localeCompare(b.nombre, 'es', { sensitivity: 'base' }));
    ...
    vehiculosFiltrados.forEach((v) => {
        ...
        fila.querySelector(".btn-editar").onclick = () => {
            ...
            editIndex = index; // Guardamos la posición que estamos editando
        ...
        fila.querySelector(".btn-email").onclick = async () => {
            ...
            try {
                ...
                alert("Error al enviar email: " + (error.text || JSON.stringify(error)));
            }
        };
...
btnMostrarForm.addEventListener("click", () => {
    editIndex = -1; // Modo nuevo registro
    form.reset();
